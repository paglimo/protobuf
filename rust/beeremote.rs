...
pub mod bee_remote_server {
    ...
    #[async_trait]
    pub trait BeeRemote: std::marker::Send + std::marker::Sync + 'static {
        async fn submit_job(
            &self,
    ...
    impl<T> Clone for BeeRemoteServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "beeremote.BeeRemote";
    impl<T> tonic::server::NamedService for BeeRemoteServer<T> {
